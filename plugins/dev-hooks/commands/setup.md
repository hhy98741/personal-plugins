...
3. If the file already contains a `## Begin dev-hooks environment variables` block, **replace everything** between `## Begin dev-hooks environment variables` and `## End dev-hooks environment variables` (inclusive) with the new block below.
4. If no existing block is found, append the block to the end of the file.

```env

## Begin dev-hooks environment variables
ENGINEER_NAME=$1
LOG_LEVEL=$2
## End dev-hooks environment variables
```
...
> ```
>
> Or if you already have a `.env` file with other variables, manually copy the `dev-hooks` block from `.env.example` into your `.env` file.
>
...
If you prefer to configure the plugin manually instead of running `/dev-hooks:setup`:
...
4. Copy the variables to your actual `.env` file:
    ...
    cp .env.example .env
    ```
    Or if you already have a `.env` with other variables, manually copy the `dev-hooks` block into it.

## Notes

- Running `/dev-hooks:setup` again will update existing settings in `.env.example` in-place rather than creating duplicates.
